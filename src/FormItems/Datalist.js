...
export class Datalist{
    ...
    set(key, value){
        ...
        if (!this.datalist) this.datalist = this.item.find("datalist");
        ...
        switch (key){
            ...
            case "maxlength":
                ...
                break;
            case "field_size":
                const sizes = Datalist.fieldSizes;
            ...
            case "options":
                ...
                value.forEach(option => {
                    ...
                    this.datalist.append(new Option(option[0], option[1]).getItem());
                });
                break;
        }
    }
    ...
    get(key){
        ...
        switch (key){
            ...
            case "options" :
                value = [];
                this.datalist.children().each((index, element) => {
                    element = $(element);
    ...
    new({name="", id="", size=6, label_text="Даталист", placeholder = "Даталист", maxlength = null, field_size = 0, required = true, disabled = false, value = "", options = ["Выбор1", "Выбор2", "Выбор3"]} = {}){
        let datalist_id = Uniqid.get("data-");
        this.input = $("<input>").attr("type", "text").attr("list", datalist_id);
        this.label = $("<label>").addClass("form-label");
        this.datalist = $("<datalist>").attr("id", datalist_id);

        this.item = Case.add([this.label, this.input, this.datalist], size);

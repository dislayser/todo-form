...
export class Textarea{
    ...
    set(key, value){
        if (!this.input) this.input = this.item.find("textarea");
        ...
        switch (key){
            ...
            case "rows":
                ...
                break;
            case "field_size":
                const sizes = Textarea.fieldSizes;
    ...
    getAll(inputs = ["name", "id", "label_text", "required", "field_size", "text", "maxlength", "size", "rows", "placeholder", "disabled"]){
        let data = {};
    ...
    getConfigInputs(){
        ...
        let rows_input = $("<input>").addClass(["form-control", "form-control-sm"]).attr({
            name : "rows",
            min : 1,
    ...
    new({name="", id="", size=6, label_text="Тестовое поле", placeholder = "Тестовое поле", maxlength = 0, field_size = 0, required = true, disabled = false, text = "", rows = 3} = {}){
        this.input = $("<textarea>").attr("type", "text");
